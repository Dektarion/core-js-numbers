...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const xABS = Math.abs(Math.sqrt(x1 ** 2 + y1 ** 2));
  const yABS = Math.abs(Math.sqrt(x2 ** 2 + y2 ** 2));
  const cos = sc / (xABS * yABS);
  const arcos = Math.acos(cos);
...
function getSumOfDigits(num) {
  ...
  const sum = arrNum.reduce((acc, elem) => acc + +elem, 0);
  return sum;
}
